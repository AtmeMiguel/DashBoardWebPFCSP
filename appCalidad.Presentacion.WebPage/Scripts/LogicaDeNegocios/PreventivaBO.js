...
var app = new Vue({
    ...
    methods: {
        ...
        iniciar: function (item) {
            ...
            axios.post(this.SERVER_URL + '/api/Claro/ListarOfrecimiento', {
                ...
                if (response.data.length > 0) {
                    ...
                    this.ListaBandeja = [];
                    this.coneccion({ CONTENIDO: 'PREV. BO | Gestion', ID: this.ID });
                }
            }).catch(e => {
                this.mostrarToast({ toast: 'warning', msg: 'Error en la conexión ...' });
            });
                ...
                if (msg.length > 34) {
                    ...
                    axios.post(this.SERVER_URL + '/api/Claro/AdministrarPreventiva', {
                        ...
                        if (response.data.ID > 0) {
                            ...
                            document.location.href = '../Claro/PreventivaBO';
                        } else {
                            this.mostrarToast({ toast: 'danger', msg: response.data.RESULTADO });
                        }
                    }).catch(e => {
                        this.mostrarToast({ toast: 'warning', msg: 'Error en la conexión ...' });
                    });
        ...
        cargarBandeja: function () {
            axios.post(this.SERVER_URL + '/api/Claro/ListarBandejaPreventiva', {
                TIPO: 5,
            }).then(response => {
                if (response.data.length > 0) {
